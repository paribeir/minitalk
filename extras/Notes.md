...
PID is the *process ID* of the current process, a unique decimal number that can be used, for example, to specify the process when attaching a debugger to it. You might have seen this number before when you, for example, accessed the system manager (Unix) or task manager (Windows). There, the PID of a certain process can be found under the Details/ID tab.
